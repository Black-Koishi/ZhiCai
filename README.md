...
| 📄 采购订单 PDF | LLM 撰写正式采购订单正文，fpdf2 生成中文 PDF，支持一键下载 |
...
```
┌────────────────────────── React 前端 (Vite + TS + Tailwind + shadcn/ui) ──────────────────────────┐
│  首页聊天 · 仪表盘 · 邮件 · 订单 · 预测 · 数据库浏览器 · 新建订单 · 设置 · 文档                      │
└──────────────────────────────────────────┬─────────────────────────────────────────────────────────┘
                                           │ REST (fetch)
┌──────────────────────────────────────────▼─────────────────────────────────────────────────────────┐
│                                   FastAPI（backend/api.py）                                        │
│   /chat · /emails · /procurement · /orders · /forecast · /database · /settings                     │
└──────────────────────────────────────────┬─────────────────────────────────────────────────────────┘
                                           │
                              ┌────────────▼────────────┐
              ...
              ┌─────────────────┬──────────┼──────────┬─────────────────┐
              ▼                 ▼          ▼          ▼                 ▼
       编排器 (orchestrator)  邮件智能体  合规智能体  PDF 生成      预测智能体
       意图路由 → JSON          LLM 提取    规则守门    固定模板 +      Prophet + LLM
       ui_actions                              + LLM 解释   fpdf2 生成 PDF
              └─────────────────┴──────────┴──────────┴─────────────────┘
                                           │
                              ┌────────────▼────────────┐
...
- **编排器（Orchestrator）**：接收用户输入，用 LLM 输出 JSON 决策（`email` / `compliance` / `pdf` / `unknown`），并附带 `ui_actions` 指令前端执行界面操作。
- **邮件智能体（Email Agent）**：解析邮件正文，用 Pydantic 约束的 LLM 提取结构化字段，再按 SKU / 关键词模糊匹配到 `items` 与 `vendors` 表。
- **合规智能体（Compliance Agent）**：执行 3 类守门检查（库存、预算、政策），并把「通过 / 失败」翻译成通俗易懂的建议。
- **预测智能体（Forecast Agent）**：对历史订单运行 Prophet 季节性分析，再让 LLM 综合为结构化洞察报告。
